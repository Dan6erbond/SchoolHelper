...
class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    public MainThreadCallback<T> getMainThreadProxy(final MainThreadCallback<T> callback) {
        return new MainThreadCallback<T>() {
            ...
            private Runnable mMainThreadRunnable = new C02841();
            ...
            class C02841 implements Runnable {
                ...
                public void run() {
                    SyncQueueItem msg = C02851.this.mQueue.next();
                    while (msg != null) {
                        switch (msg.what) {
                            case 1:
    ...
    public BackgroundCallback<T> getBackgroundProxy(final BackgroundCallback<T> callback) {
        return new BackgroundCallback<T>() {
            ...
            private Runnable mBackgroundRunnable = new C02861();
            ...
            class C02861 implements Runnable {
                ...
                public void run() {
                    while (true) {
                        SyncQueueItem msg = C02872.this.mQueue.next();
                        if (msg != null) {
                            switch (msg.what) {
                                case 1:
                                    C02872.this.mQueue.removeMessages(1);
                                    callback.refresh(msg.arg1);
                                    break;
                                case 2:
                                    C02872.this.mQueue.removeMessages(2);
                                    C02872.this.mQueue.removeMessages(3);
                        ...
                        C02872.this.mBackgroundRunning.set(false);
                        return;
                    }
                }
            }
