...
class AppCompatCompoundButtonHelper {
    ...
    void loadFromAttributes(AttributeSet attrs, int defStyleAttr) {
        TypedArray a = this.mView.getContext().obtainStyledAttributes(attrs, C0259R.styleable.CompoundButton, defStyleAttr, 0);
        try {
            if (a.hasValue(C0259R.styleable.CompoundButton_android_button)) {
                int resourceId = a.getResourceId(C0259R.styleable.CompoundButton_android_button, 0);
                if (resourceId != 0) {
                    this.mView.setButtonDrawable(this.mDrawableManager.getDrawable(this.mView.getContext(), resourceId));
                }
            }
            if (a.hasValue(C0259R.styleable.CompoundButton_buttonTint)) {
                CompoundButtonCompat.setButtonTintList(this.mView, a.getColorStateList(C0259R.styleable.CompoundButton_buttonTint));
            }
            if (a.hasValue(C0259R.styleable.CompoundButton_buttonTintMode)) {
                CompoundButtonCompat.setButtonTintMode(this.mView, DrawableUtils.parseTintMode(a.getInt(C0259R.styleable.CompoundButton_buttonTintMode, -1), null));
            }
            a.recycle();
        } catch (Throwable th) {
            a.recycle();
