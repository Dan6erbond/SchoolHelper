...
class AppCompatBackgroundHelper {
    ...
    void loadFromAttributes(AttributeSet attrs, int defStyleAttr) {
        TypedArray a = this.mView.getContext().obtainStyledAttributes(attrs, C0259R.styleable.ViewBackgroundHelper, defStyleAttr, 0);
        try {
            if (a.hasValue(C0259R.styleable.ViewBackgroundHelper_android_background)) {
                ColorStateList tint = this.mDrawableManager.getTintList(this.mView.getContext(), a.getResourceId(C0259R.styleable.ViewBackgroundHelper_android_background, -1));
                if (tint != null) {
                    setInternalBackgroundTint(tint);
                }
            }
            if (a.hasValue(C0259R.styleable.ViewBackgroundHelper_backgroundTint)) {
                ViewCompat.setBackgroundTintList(this.mView, a.getColorStateList(C0259R.styleable.ViewBackgroundHelper_backgroundTint));
            }
            if (a.hasValue(C0259R.styleable.ViewBackgroundHelper_backgroundTintMode)) {
                ViewCompat.setBackgroundTintMode(this.mView, DrawableUtils.parseTintMode(a.getInt(C0259R.styleable.ViewBackgroundHelper_backgroundTintMode, -1), null));
            }
            a.recycle();
        } catch (Throwable th) {
            a.recycle();
    ...
    private boolean shouldCompatTintUsingFrameworkTint(@NonNull Drawable background) {
        return VERSION.SDK_INT == 21 && (background instanceof GradientDrawable);
    }
